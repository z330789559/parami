...
fn withdraw_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Bridge::transfer(Origin::signed(ALICE), tx_hash_transfer(), eth_addr(), value));

		assert_eq!(0, free_balance(&BOB));
		assert_ok!(Bridge::withdraw(Origin::signed(ALICE), tx_hash_withdraw1(), eth_addr(), BOB, 200));
		assert_ok!(Bridge::withdraw(Origin::signed(ALICE), tx_hash_withdraw1(), eth_addr(), BOB, 200));
		assert_eq!(200, free_balance(&BOB));
		assert_eq!(last_event(), Event::parami_bridge(crate::Event::Withdraw(tx_hash_withdraw1(), true)));

		assert_ok!(Bridge::withdraw(Origin::signed(ALICE), tx_hash_withdraw2(), eth_addr(), BOB, 300));
		assert_eq!(200, free_balance(&BOB));
		assert_eq!(last_event(), Event::parami_bridge(crate::Event::Withdraw(tx_hash_withdraw2(), false)));

		assert_ok!(Bridge::withdraw(Origin::signed(ALICE), tx_hash_withdraw3(), eth_addr(), BOB, value-200));
		assert_eq!(value, free_balance(&BOB));
		assert_eq!(last_event(), Event::parami_bridge(crate::Event::Withdraw(tx_hash_withdraw3(), true)));
	});
}
...
fn withdraw_should_fail() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(0, free_balance(&BOB));
		assert_ok!(Bridge::withdraw(Origin::signed(ALICE), vec![11], vec![22], BOB, 444));
		assert_eq!(last_event(), Event::parami_bridge(crate::Event::Withdraw(vec![11], false)));
		assert_eq!(0, free_balance(&BOB));
		assert_eq!(0, Bridge::erc20_balances(vec![22]));
	});
}
...
fn transfer_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Bridge::transfer(Origin::signed(ALICE), tx_hash(), eth_addr(), value));
		assert_eq!(last_event(), Event::parami_bridge(crate::Event::Transfer(tx_hash())));
		assert_eq!(Some(crate::Erc20Event::Transfer {
			value,
			from: eth_addr(),
		...
		assert_ok!(Bridge::transfer(Origin::signed(ALICE), tx_hash2(), eth_addr(), value));
		assert_eq!(last_event(), Event::parami_bridge(crate::Event::Transfer(tx_hash2())));
