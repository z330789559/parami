...
use sp_runtime::traits::{
    AccountIdConversion, AtLeast32BitUnsigned, CheckedAdd, One, SaturatedConversion, Saturating,
    Zero,
};
use sp_std::{
...
use parami_traits::{
    Balance, CurrencyId, MultiCurrency, MultiCurrencyExtended, MultiLockableCurrency, PalletId,
};
...
type BalanceOf<T> =
    <<T as Config>::Tokens as MultiCurrency<<T as frame_system::Config>::AccountId>>::Balance;

type CurrencyIdOf<T> =
    <<T as Config>::Tokens as MultiCurrency<<T as frame_system::Config>::AccountId>>::CurrencyId;
...
pub mod swap_module {
    ...
    pub type TokenToSwap<T: Config> =
        StorageMap<_, Twox64Concat, CurrencyIdOf<T>, CurrencyIdOf<T>, ValueQuery>;

    /// The total issuance of a token type.
    #[pallet::storage]
    #[pallet::getter(fn swaps_house)]
    pub type SwapsHouse<T: Config> = StorageMap<
        _,
        Twox64Concat,
        CurrencyIdOf<T>,
        Option<Swap<T::AccountId, CurrencyIdOf<T>>>,
        ValueQuery,
    >;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_swap(
            ...
            let sender = ensure_signed(origin)?;
                ...
                !TokenToSwap::<T>::contains_key(token_id),
                Error::<T>::SwapAlreadyExists
            );

            let swap_id = Self::swap_count();
            let next_id = swap_id.checked_add(&One::one()).ok_or("Overflow")?;

            let swap_token_id =
                T::Tokens::create_token(&sender, Zero::zero(), &symbol, big_decimal)
                    .map_err(|_| Error::<T>::SwapAlreadyExists)?;
        ...
        pub fn add_liquidity(
            ...
            if let Some(swap) = Self::swaps_house(swap_id) {
                ...
                if total_liquidity > Zero::zero() {
                        ...
                        min_liquidity > Zero::zero(),
                        Error::<T>::RequestedZeroLiquidity
                    );
                    ...
                    let liquidity_minted = currency_amount * total_liquidity / swap_balance;
                        ...
                        Error::<T>::TooLowLiquidity
                    );

                    T::Currency::transfer(
                        &who,
                        &swap.account,
                        Self::convert(currency_amount),
                    ...
                    let initial_liquidity =
                        T::Currency::free_balance(&this).saturated_into::<u64>();
        ...
        pub fn remove_liquidity(
            ...
            if let Some(swap) = Self::swaps_house(swap_id) {
                ...
                let currency_amount = shares_to_burn.clone() * Self::unconvert(swap_balance)
        ...
        #[pallet::weight(0)]
        pub fn currency_to_tokens_input(
            origin: OriginFor<T>,
            swap_id: CurrencyIdOf<T>,
            ...
            if let Some(swap) = Self::swaps_house(swap_id) {
                ...
                let swap_balance = Self::get_swap_balance(&swap);
                let tokens_bought =
                    Self::get_input_price(currency, Self::unconvert(swap_balance), token_reserve);
        ...
        /// tokens to be returned.
        #[pallet::weight(0)]
        pub fn currency_to_tokens_output(
            origin: OriginFor<T>,
            swap_id: CurrencyIdOf<T>,
        ...
        #[pallet::weight(0)]
        pub fn tokens_to_currency_input(
            origin: OriginFor<T>,
            swap_id: CurrencyIdOf<T>,
        ...
        #[pallet::weight(0)]
        pub fn tokens_to_currency_output(
            origin: OriginFor<T>,
            swap_id: CurrencyIdOf<T>,
            ...
            let buyer = ensure_signed(origin)?;
                ...
                currency_bought > Zero::zero(),
                Error::<T>::NoCurrencySwapped
            );
    ...
    impl<T: Config> Pallet<T> {
        pub fn get_currency_to_token_input_price(
            swap: &Swap<T::AccountId, CurrencyIdOf<T>>,
        ...
        fn get_input_price(
            ...
            let denominator =
                (input_reserve * Self::convert_balance(1000u128)) + input_amount_with_fee;
