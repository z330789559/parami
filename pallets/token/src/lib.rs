...
use codec::{Decode, Encode, EncodeLike, HasCompact};
use frame_support::{
    ensure,
    pallet_prelude::*,
    sp_tracing,
...
use parami_traits::{
    ...
    Balance, BalanceStatus, CurrencyId, GetByKey, LockIdentifier, MultiCurrency,
    MultiCurrencyExtended, MultiLockableCurrency, MultiReservableCurrency, OnDust, PalletId,
...
use sp_runtime::{
    traits::{
        ...
        MaybeSerializeDeserialize, Member, One, Printable, Saturating, StaticLookup, Zero,
    },
    AccountId32, DispatchError, DispatchResult, RuntimeDebug, TypeId,
};
use sp_std::{
...
impl DefaulCurrency {
    ...
    pub fn getDefaultToken<T>(index: T) -> Self
    where
        T: Into<u32>,
    {
        match index.into() {
...
pub mod module {
    ...
    pub type TokenInfo<T: Config> =
        StorageMap<_, Twox64Concat, Vec<u8>, TokenData<T::AccountId, T::CurrencyId>, ValueQuery>;
            ...
            self.endowed_accounts
                ...
                .for_each(|(account_id, currency_id, initial_balance)| {
                    ...
                    if DefaulCurrency::UNKOWN != DefaulCurrency::getDefaultToken(*currency_id) {
                        DefaulCurrency::getDefaultToken(*currency_id)
                            .getTokenInfo()
                            .map(|(ref symbol, index, bigDecimal)| {
                                TokenInfo::<T>::insert(
                                    symbol,
                                    TokenData {
...
impl<T: Config> MultiCurrency<T::AccountId> for Pallet<T> {
    ...
    fn create_token(
        ...
        system_symbol: &Vec<u8>,
        bigDecimal: u32,
    ) -> TokenResult<T::CurrencyId> {
